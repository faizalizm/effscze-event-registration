...
<html lang="en">
  <head>
    ...
    <script>
      ...
      {
        if(confirm('Are you sure to remove user with ID : ' + id + ' ?'))
        {
  ...
  <body>
    ...
    <?php
      if(isset ($_SESSION["admin"])) 
      {
        ...
        $listWebCommand = "SELECT * FROM user";
        $executeListWeb = mysqli_query($connect, $listWebCommand);
        ...
        if(isset($_POST["keyword"])){
          ...
          $findCommand = "SELECT * FROM user WHERE $filterBy LIKE '%$key%'";
    ...
    ?> 
      ...
      <div class="div1" id="scroll">
        ...
        <div class="tab-link" id="scroll-to-view">
          <div class="tab">
            <h4>
              Showing Registered User - <span class="highlight-text">ABSTRAKT Member</span>
            </h4>
          </div>
        </div> 
        <div class="table" >
          <form action="adminViewUser.php#scroll-to-view" class="table-search" method="POST">
            ...
            <select name="filterBy" title="Choose from some options to filter" >
              <option value="userid">Filter by ID</option>
                ...
                if($counter % 2 == 0){
                  echo "
                    <tr class='light'>
                      ...
                      <td class='del-box'>
                        <a class='del-btn tab-link' href='javascript:delete_id($searched[userid])'>Delete</a>
                      </td>
                    </tr>
                  ";
                }
        ...
        <hr class="hr-margin">
        <h6 class='event-heading search-result search-margin'>List of Members</h6>
        <table border="0">
          <tr>
            <th class="id-col">Participant ID</th>
            <th class="user-col">Username</th>
          ...
          <?php
            $counter = 0;
            while($row = $executeListWeb->fetch_assoc()) {
              if($counter % 2 == 0){
                echo "
                  <tr class='light'>
                    ...
                    <td class='del-box'>
                      <a class='del-btn tab-link' href='javascript:delete_id($row[userid])'>Delete</a>
                    </td>
                  </tr>
                ";
                }
